...
def run():
    # We need to use quadruple braces '{{{{' in f-strings to produce double braces '{{' in the output.
    # This is required for Frappe to recognize them as Jinja tags.
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 40px;">
            ...
            <div style="width: 48%;">
                ...
                <p style="font-size: 14px; font-weight: 600; margin: 0 0 5px 0; color: #111827;">
                    {{{{ doc.customer_name or doc.supplier_name or doc.party_name }}}}
                </p>
    ...
    # Do not rename Company, only apply Print Formats
    
    # 1. Update Print Formats
    targets = ["Sales Invoice", "Sales Order", "Purchase Order", "Quotation"]
    ...
    for doctype in targets:
        ...
        if not frappe.db.exists("Print Format", fmt_name):
            ...
            pf.save()
            print(f"Created {fmt_name}")
        else:
            pf = frappe.get_doc("Print Format", fmt_name)
            pf.html = html_template
            pf.save()
            print(f"Updated {fmt_name}")

    frappe.db.commit()
    print("Verification: Print Formats updated with correct quadruple-escape sequence.")
